...
void callback_light(CoapPacket &packet, IPAddress ip, int port) {
  ...
  LEDSTATE = message.equals("1") ? true : false;
  digitalWrite(2, LEDSTATE ? HIGH : LOW);
  coap.sendResponse(ip, port, packet.messageid, LEDSTATE ? "1" : "0");
...
void loop() {
  ...
  if(digitalRead(0) == LOW) {
    int msgid = coap.put(IPAddress(192, 168, 43, 157), 5683, "light","1");
    Serial.println(msgid);
  } else if(digitalRead(0) == HIGH) {
    int msgid = coap.put(IPAddress(192, 168, 43, 157), 5683, "light","0");
    Serial.println(msgid);
  }
  coap.loop();  
}
